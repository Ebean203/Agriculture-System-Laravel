        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                ...
                <div class="mt-2 text-sm text-gray-600">
                    ...
                    Total RSBSA Registered Farmers: <span class="font-bold text-agri-green">{{ number_format($totalRecords ?? 0) }}</span>
                </div>
            </div>
            <div class="flex flex-col sm:flex-row gap-3">
                <a href="{{ route('reports.index', ['prefill' => 'rsbsa']) }}" class="bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 transition-colors flex items-center"
                   onclick="event.preventDefault(); exportRsbsaToPDF();">
                    <i class="fas fa-file-pdf mr-2"></i>Export to PDF
                </a>
                
                <!-- Page Navigation intentionally omitted (legacy cleanup) -->
            </div>
        </div>
    </div>
        ...
        <form method="GET" class="flex flex-col gap-4">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="md:col-span-2">
                    ...
                    <div class="relative">
                        <input type="text" name="search" id="rsbsa_search" value="{{ $search }}" 
                               ...
                               onfocus="if(this.value) searchRSBSAAutoSuggest(this.value)">
                        <i class="fas fa-search absolute left-3 top-3 text-gray-400"></i>
                        <!-- Auto-suggest dropdown -->
                        <div id="rsbsa_suggestions" class="absolute z-50 w-full bg-white border border-gray-300 rounded-lg shadow-lg mt-1 max-h-60 overflow-y-auto hidden"></div>
                    </div>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">
            ...
            <div class="flex gap-3">
                ...
                @if($search || $barangay)
                <a href="{{ route('rsbsa') }}" class="px-6 py-2 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50">Clear</a>
                @endif
            </div>
        </form>
    </div>
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-12 text-center text-gray-500">
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-sm text-gray-600 mb-2">No RSBSA-registered farmers found for your filters</p>
...
function searchRSBSAAutoSuggest(query) {
    ...
    fetch(`/api/farmers/search?query=${encodeURIComponent(query)}&filter_type=rsbsa`)
        .then(r => r.json())
        .then(data => {
                ...
                data.farmers.forEach(farmer => {
                    ...
                    html += `
                        <div class="px-3 py-2 hover:bg-gray-100 cursor-pointer border-b last:border-b-0"
                             onclick="selectRSBSASuggestion('${farmer.farmer_id}', '${safeName}', '${farmer.contact_number || ''}')">
...
function selectRSBSASuggestion(farmerId, farmerName, contactNumber) {
    const input = document.getElementById('rsbsa_search');
    input.value = farmerName;
    setTimeout(() => document.getElementById('rsbsa_suggestions').classList.add('hidden'), 150);
    const form = input.closest('form');
    if (form) form.submit();
}

function exportRsbsaToPDF() {
    const params = new URLSearchParams(window.location.search);
    let url = '{{ route('reports.index') }}?prefill=rsbsa';
